...
- **A "principal" is a `user` or an `api_credential`, undifferentiated.** Resources can be
  ...
  to tell them apart. So `created_by`/`updated_by`/`deleted_by` (and `role_assignment`'s
  actor-facing column, and `audit_log`'s) store a single **principal id**: a `uuid` that
  points at either `user.id` or `api_credential.id`, with no database-level FK constraint
...
- **Exceptions**, where the pattern doesn't fit and isn't applied:
  - `audit_log` is append-only by design (TECHNICAL_DESIGN §9) — no `updated_by`/`deleted_at`/
    `deleted_by`. Its single `actor_id` (principal, per above) already serves the "who"
    question for that one row.
  - `verification_token` has `used_at`/`expires_at` as its own closed-lifecycle markers — no
    separate `deleted_at`.
  - `session` and `api_credential` use the exact same `deleted_at`/`deleted_by` pair as
...
Branding and auth-policy settings are folded directly into `tenant` — both are strictly 1:1
relationships, so splitting them into their own tables bought organizational tidiness but
no normalization benefit. Token TTLs are the exception: those live on `app` (§2), not here
— see that section for why.

| Column | Type | Notes |
|---|---|---|
| `id` | uuid, PK | |
| `parent_id` | uuid, FK → tenant.id, nullable | null for the root tenant |
| `name` | text | |
...
### `tenant_provider_credential` *(inferred grouping)*
...
Token lifetimes live here, not on `tenant` — they're configured per app, same as signing
...
| `app_id` | uuid, FK → app.id | points at the default system app for tenant-level sessions (TECHNICAL_DESIGN §3.5), or a real app for app-scoped sessions |
...
|---|---|---|
| `id` | uuid, PK | |
| `tenant_id` | uuid, FK → tenant.id | identity is isolated per tenant (PRD §4.2) |
...
### `password`
**Not** 1:1 with a user — a user has *many* `password` rows over time, of which exactly one
has `deleted_at IS NULL` (their current/active password); a partial unique index enforces
...
account auto-linking by verified email (PRD §5.3): linking checks `user.email` first, not
this table directly.

### `verification_token` *(inferred)*
...
|---|---|---|
| `id` | uuid, PK | |
| `tenant_id` | uuid, FK → tenant.id | shared across all apps in the tenant (PRD §6) |
| `name` | text | |
...
when" is ever needed, it's reconstructed from `audit_log` (PRD §11), not stored on the row.
...
| `id` | uuid, PK | |
| `tenant_id` | uuid, FK → tenant.id | |
| `app_id` | uuid, FK → app.id, nullable | null = org-management system role (e.g. Owner) not tied to a specific app's business catalog; set = that app's business role (PRD §7.2) |
| `org_id` | uuid, FK → organization.id, nullable | null = base role definition (app-owner-defined); set = an org's customized/extended variant |
| `name` | text | |
...
string within the role itself (`role.permissions`, e.g. `users:read@org`) — a role
...
disambiguates which org a **shared/base** role (`role.org_id` is null — reused across every
...
already org-specific (`role.org_id` set) or has neither `app_id` nor `org_id` (app-scoped
...
| `org_id` | uuid, FK → organization.id, nullable | only set when assigning a shared/base role in the context of one specific org; null otherwise |
| `created_at` | timestamp | |
...
the standard `deleted_at`/`deleted_by` pair, same as `session`.
...
`role_assignment.principal_id` (§5), decided rather than inferred now.
...
1. **`verification_token` retention** — how long expired/used tokens are kept before
