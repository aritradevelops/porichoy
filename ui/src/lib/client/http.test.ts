...
describe('apiFetch', () => {
  ...
  it('attaches the current access token as a bearer header', async () => {
    ...
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse(200, { data: {}, error: null }))
  ...
  it('throws ApiError carrying the backend error key/message', async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          jsonResponse(401, { data: null, error: { key: 'identity.unauthenticated', message: 'Nope.' } }),
        ),
    )
  ...
  it('throws a network ApiError when fetch itself rejects', async () => {
    ...
    await expect(apiFetch('/auth/me')).rejects.toMatchObject({ status: 0, key: 'network.error' })
  })
})
